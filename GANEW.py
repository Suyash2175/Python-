    ...
    with open(file_name, 'r') as f:
        for line in f:
            ...
            if city_name:
                ...
                longitude = random.uniform(60.0, 100.0)  # Random longitude for Indian cities
                latitude = random.uniform(8.0, 35.0)    # Random latitude for Indian cities
...
def calculate_total_distance(path, locations):
    ...
    total_path_dist += city_distance(locations[path[-1]], locations[path[0]])  # Returning to start
...
# Main function
def main():
    cities_data = load_cities('india_cities.txt')  # Ensure this file has one city name per line
    if not cities_data:
